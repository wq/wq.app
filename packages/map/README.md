...
Components marked with * are placeholders for implementations provided by [@wq/map-gl] and [@wq/leaflet].  The typical component hierarchy generated by `<AutoMap/>` is as follows:
...
Basemap layers are specified as part of the [global configuration](#global-configuration).  Only one Basemap component can be active at a time, and it is drawn below all other layers.  @wq/map provides a placeholder `Tile` component which is overridden by [@wq/map-gl] and [@wq/leaflet].  [@wq/map-gl] also provides a `VectorTile` basemap component, while [@wq/leaflet]'s plugins include other basemaps for use with specific mapping services.

config name | component | details
--|--|--
tile | Tile | Raster tile layer, typically with 256x256 tile images in "Web Mercator" projection
vector-tile | VectorTile | Vector tile layer (@wq/map-gl only)
...
Overlays are specified as part of the [page configuration](#page-configuration).  All overlays are active initially (unless disabled), and are drawn over the basemap.  @wq/map provides placeholder overlay components which are overridden by [@wq/map-gl] and [@wq/leaflet].
